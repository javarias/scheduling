 ...
 * <li> nothingToSchedule -- The Scheduler is in the nothing-to-schedule state and is waiting for a resoponse.
 * <li> failure -- An uncorrectable error has occurred.
 * <li> synchronous -- The mode in which the scheduler operates.  In the synchronous mode
 * 		the scheduler waits for the control system to complete the execution of a scheduling 
 * 		block before continuing its activity.  In the asynchronous mode it continues its
...
public class SchedulerConfiguration extends TaskControl {
	...
	/**
	 * A response action to indicate the scheduler should execute a specified filler program.
	 */
	static public final int FILLER = 3;
	/**
	 * A response action to indicate the scheduler should execute a specified scheduling unit.
	 */
	...
	public SchedulerConfiguration(Thread masterScheduler,
			boolean dynamic, boolean synchronous, SBQueue queue, int bestNumber, int sleepTime,
			int subarrayId, Clock clock, Control control,
	...
	// set by the MasterScheduler when creating a Scheduler and never changed after that.
