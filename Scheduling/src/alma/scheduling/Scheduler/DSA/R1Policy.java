 ...
 * <p>
 * The algorithm divides into two computations: a success 
 * factor that is based on position and weather and a ranking
 *  factor that ranks scheduling blocks relative to each other. 
 * A final score is computed by taking the product of the success 
 * factor and the ranking factor. 
 * <p>
 * The sucess factor includes three factors (positionElevation, 
 * positionMaximum, weather), each with its own weight. There are two 
 * position factors. The first is the sin of the current elevation of 
 * the target, or zero if the target is not currently visible. Thus a 
 * target is penalized if its elevation is low. The second position factor 
 * is related to how close the target is to its maximum elevation. The 
 * idea is to give a high weight to a position if it is close to the 
 * maximum elevation of the source. A delta, in units of hours, is 
 * computed that is the absolute value of the difference between the 
 * current LST and the LST at the maximum elevation. If this delta is 
 * between 0 and 30 minutes, the function returns 1; if between 30 minutes and 1 hour, 
 * 0.9 is returned; if between 1 hour and 90 minutes, 0.8 is returned; 
 * otherwise 0 is returned. The third factor is the result of evaluating 
 * any weather constraint. 
 * <p>
 * The ranking factors are listed below and are fairly straightforward, 
 * but their associated weights can greatly change the decision-making process. 
 * <ul>
 * <li>priority - The scientific priority (This priority varies from 10 (highest) to 1). 
 * <li>sameProjectSameBand - 1 if this scheduling block's project and frequency band 
 * are the same as the immediately preceeding scheduling block; otherwise 0. 
 * <li>sameProjectDifferentBand - 1 if this scheduling block's project is the same but 
 * its frequency band is different from the immediately preceding scheduling block; otherwise 0. 
 * <li>differentProjectSameBand - 1 if this scheduling block's frequency band is the 
 * same but its project is different from the immediately preceding scheduling block; otherwise 0. 
 * <li>differentProjectDifferentBand - 1 if this scheduling block's project and 
 * its frequency band are different from the immediately preceding scheduling block; otherwise 0. 
 * <li>newProject - 1 if this scheduling block is from a project that has not been started; otherwise 0. 
 * <li>oneSBRemaining - 1 if this scheduling block is the only scheduling block that 
 * remains in a project; otherwise 0. 
</ul>
Each ranking factor is computed and multiplied by its weight; the sum of 
all ranking factors forming the final rank. The final score is computed 
by multiplying the final ranking by the final success factor. The scheduling 
block with the highest final score is selected for execution, provided that 
...
class R1Policy extends PolicyType {
	...
	public R1Policy (int subarrayId, Policy policy, SBQueue queue, 
		...
		unit = new R1Unit [sb.length];
		SiteCharacteristics site = telescope.getSite();
		for (int i = 0; i < unit.length; ++i) {
			unit[i] = new R1Unit (sb[i],site);
		}
	...
	public BestSB getBest() throws SchedulingException {
		...
		if (i == unit.length){
            System.out.println("i == unit.length");
			return null;
		}
		// Compute the score.
		score();
	...
	private R1Unit[] topList() {
		...
		R1Unit[] out = new R1Unit [bestSize];
		for (int i = 0; i < bestSize; ++i)
			out[i] = unit[copy[i].a];
	...
	private void setState(R1Unit u, String currentProjectId, FrequencyBand currentBand) {
		SB sb = u.getSB();
