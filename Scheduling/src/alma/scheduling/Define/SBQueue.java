...
public class SBQueue {
    ...
    public synchronized String[] getAllReadyIds() {
        SB[] ready = getReady();
        String[] x = new String[ready.length];
    ...
    public synchronized void replace (SB sb) {
        for(int i=0; i < queue.size(); i++){
            if(sb.getId().equals( ((SB)queue.get(i)).getId() ) ){
                queue.set(i, sb);
            }
        }
    }
