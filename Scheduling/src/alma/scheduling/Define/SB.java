 ...
 * 
 * @version $Id: SB.java,v 1.24 2007/07/24 20:49:32 sslucero Exp $
 * @author Allen Farris
 */
public class SB implements ProgramMember {
	...
	public void printTree(PrintStream out, String indent) {
		out.println(indent + "SB: "+sbName+": " + schedBlockId);
			...
			if (ex != null) {
				...
				if (ex.getStatus().isComplete()) {
                    ...
                    if(getIndefiniteRepeat() ){
						...
						status.setEnded(time,Status.OBSERVED);
    ...
    public void setAborted(DateTime t){
        status.setEnded(t, Status.ABORTED);
        parent.execEnd(this, t, 0, Status.ABORTED);
    }
    ...
    public void setIndefiniteRepeat(boolean b) {
        indefiniteRepeat =b;
    }
